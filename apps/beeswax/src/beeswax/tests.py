...
""")
  ...
  def test_query_with_setting(self):
    ...
    response = _make_query(self.client, "CREATE TABLE `%(db)s`.`%(table_name)s` AS SELECT foo+1 FROM test WHERE foo=4" % {
      'db': self.db_name, 'table_name': table_name},
      # Run on MR, because that's how we check it worked.
      settings=[("mapred.job.name", "test_query_with_setting"),
        ("hive.exec.compress.output", "true")], local=False, database=self.db_name) 
    response = wait_for_query_to_finish(self.client, response, max=180.0)
  ...
  def test_query_with_udf(self):
    ...
    for engine in execution_engines:
      response = _make_query(self.client, "SELECT my_sqrt(foo), my_float(foo) FROM test where foo=4 GROUP BY foo",
        # Force MR job with GROUP BY
  ...
  def test_parameterization(self):
    ...
    response = _make_query(self.client, "SELECT foo FROM test WHERE foo='$x' and bar='$y'",
                           params=[('x', '1'), ('y', '2')], database=self.db_name)
    content = json.loads(response.content)
    assert_true('watch_url' in content, content)
  ...
  def test_explain_query(self):
    ...
    response = _make_query(c, "SELECT SUM(foo) FROM `%(db)s`.`test_explain`" % {'db': self.db_name},
                           settings=[('hive.explain.user', 'false')], submission_type="Explain") # Need to prefix database in Explain
  ...
  def test_analyze_table_and_read_statistics(self):
    ...
    try:
      ...
      response = self.client.post(reverse("beeswax:analyze_table", kwargs={'database': self.db_name,
                                                                           'table': 'test', 'columns': True}), follow=True)
      response = wait_for_query_to_finish(self.client, response, max=120.0)
      assert_true(response, response)
  ...
  def test_get_top_terms(self):
    ...
    resp = self.client.get(reverse("beeswax:get_top_terms", kwargs={'database': self.db_name,
                                                                    'table': 'test', 'column': 'foo', 'prefix': '10'}))
    ...
    resp = self.client.get(reverse("beeswax:get_top_terms", kwargs={'database': self.db_name,
                                                                    'table': 'test', 'column': 'foo', 'prefix': '10'}) + '?limit=2')
  ...
  def test_beeswax_api_autocomplete(self):
    ...
    resp = self.client.get(reverse("beeswax:api_autocomplete_column", kwargs={'database': self.db_name,
                                                                              'table': 'nested_table', 'column': 'foo'}))
    json_resp = json.loads(resp.content)
    ...
    resp = self.client.get(reverse("beeswax:api_autocomplete_nested", kwargs={'database': self.db_name,
                                                                              'table': 'nested_table', 'column': 'foo', 'nested': 'item'}))
    json_resp = json.loads(resp.content)
    assert_false('error' in json_resp, 'Failed to autocomplete nested type: %s' % json_resp.get('error'))
...
def test_hive_site_host_pattern_local_host():
  ...
  try:
    ...
    xml = hive_site_xml(is_local=False, use_sasl=False, thrift_uris=thrift_uris,
                        kerberos_principal='test/_HOST@TEST.COM', hs2_kerberos_principal='test/_HOST@TEST.COM')
    open_file(os.path.join(tmpdir, 'hive-site.xml'), 'w').write(xml)
...
def test_search_log_line():
  logs = """
    ...
    2012-08-18 12:23:15,648 ERROR [pool-1-thread-2] ql.Driver (SessionState.java:printError(380)) - FAILED: 
    Parse Error: line 1:31 cannot recognize input near '''' '_this_is_not' 'SQL' in constant
  ...
  logs = "12/08/22 20:50:14 ERROR ql.Driver: FAILED: Parse Error: line 1:31 cannot" \
         "recognize input near '''' '_this_is_not' 'SQL' in constant'"
  assert_true(search_log_line('FAILED: Parse Error', logs))

  logs = """
    FAILED: Parse Error: line 1:31 cannot recognize input near '''' '_this_is_not' 'SQL' in constant
    2012-08-18 12:23:15,648 ERROR [pool-1-thread-2] ql.Driver (SessionState.java:printError(380)) - FAILED:
    Parse XXXX Error: line 1:31 cannot recognize input near '''' '_this_is_not' 'SQL' in constant
    org.apache.hadoop.hive.ql.parse.ParseException: line 1:31 cannot recognize input near '''' '_this_is_not' 'SQL' in constant
    """
  ...
  def __init__(self, describe=None):
    if describe is not None:
      ...
      self.describe = [
            ...
            {'comment': None, 'col_name': 'Location:           ',
             'data_type': 'hdfs://localhost:8020/user/hive/warehouse/test_partitions'},
            ...
            {'comment': None, 'col_name': 'OutputFormat:       ',
  ...
  def __init__(self, describe=None):
    if describe is not None:
      ...
      self.describe = [
        ...
        {'comment': 'NULL', 'col_name': 'OutputFormat:       ',
         'data_type': 'org.apache.hadoop.hive.ql.io.HiveIgnoreKeyTextOutputFormat'},
...
class TestHiveServer2API(object):
  ...
  def test_column_format_values_nulls(self):
    ...
    assert_equal([None, 1, 1, 1, 1, 1, 1, 1, None, None, 1, 1, 1, None, 1, 1, None, 1, None, 1, 1, 1, None,
                  1, None, None, None, 1, 1, None, None, 1, None, 1, 1, None, 1, 1, 1, None, None, None, 1,
                  None, 1, None, 1, None, None, 1, None, None, 1, 1, None, None, None, None, None, None, 1, None, None, None],
                 HiveServerTColumnValue2.set_nulls(data, nulls))
...
class TestWithMockedServer(object):
  ...
  def test_save_design(self):
    with patch('beeswax.server.dbms.KazooClient') as KazooClient:
      ...
      try:
        _save_design(user=self.user_not_me, design=design, type_=HQL, design_obj=design_obj,
                     explicit_save=True, name='test_save_design', desc='test_save_design desc')
      ...
      saved_design = _save_design(user=self.user_not_me, design=design, type_=HQL, design_obj=design_obj,
                                  explicit_save=False, name='test_save_design', desc='test_save_design desc')
      assert_equal('test_save_design (new)', saved_design.name)
      assert_equal('test_save_design desc', saved_design.desc)
      assert_equal('test_save_design (new)', saved_design.doc.get().name)
      ...
      # Save query
      saved_design = _save_design(user=self.user, design=design, type_=HQL, design_obj=design_obj,
                                  explicit_save=True, name='This__design__name__contains___sixty__five___characters___exactly',
                                  desc='test_save_design desc')
      len_after = len(saved_design.name)
      assert_equal(len_after, 64)
      saved_design = _save_design(user=self.user, design=design, type_=HQL, design_obj=design_obj,
                                  explicit_save=False, name='This__design__name__contains___sixty__five___characters___exactly',
                                  desc='test_save_design desc')
  ...
  def test_get_history_xss(self):
    with patch('beeswax.server.dbms.KazooClient') as KazooClient:
      ...
      sql_escaped = b'SELECT count(sample_07.salary) FROM ' \
                    b'sample_07;&quot;&gt;&lt;iFrAME&gt;src=&quot;javascript:alert(&#39;Hue has an xss&#39;);&quot;&gt;&lt;/iFraME&gt;'
...
def test_hiveserver2_jdbc_url():
  ...
  try:
    ...
    url = hiveserver2_jdbc_url()
    assert_equal(url, 'jdbc:hive2://server-with-ssl-enabled.com:10000/default;ssl=true;'
                      'sslTrustStore=/path/to/truststore.jks;trustStorePassword=password')
    ...
    assert_equal(url, 'jdbc:hive2://server-with-ssl-enabled.com:10000/default;ssl=true;'
                      'sslTrustStore=/etc/ssl-conf/CA_STANDARD/truststore.jks')
...
def test_sasl_auth_in_large_download():
  ...
  create_sql = "CREATE TABLE IF NOT EXISTS %(db)s.%(table_name)s (w0 CHAR(8),w1 CHAR(8)," \
               "w2 CHAR(8),w3 CHAR(8),w4 CHAR(8),w5 CHAR(8),w6 CHAR(8),w7 CHAR(8),w8 CHAR(8),w9 CHAR(8))" % table_info
  hql = string_io()
